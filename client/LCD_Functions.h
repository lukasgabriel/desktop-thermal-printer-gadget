...
*/
const int scePin0 = 17;   // SCE - Chip select, pin 3 on LCD.
const int rstPin0 = 21;   // RST - Reset, pin 4 on LCD.
const int dcPin0 = 26;    // DC - Data/Command, pin 5 on LCD.
const int sdinPin0 = 19;  // DN(MOSI) - Serial data, pin 6 on LCD.
const int sclkPin0 = 18;  // SCLK - Serial clock, pin 7 on LCD.
const int blPin0 = 28;    // LED - Backlight LED, pin 8 on LCD.

const int scePin1 = 13;   // SCE - Chip select, pin 3 on LCD.
const int rstPin1 = 20;   // RST - Reset, pin 4 on LCD.
const int dcPin1 = 27;    // DC - Data/Command, pin 5 on LCD.
const int sdinPin1 = 15;  // DN(MOSI) - Serial data, pin 6 on LCD.
const int sclkPin1 = 14;  // SCLK - Serial clock, pin 7 on LCD.
const int blPin1 = 22;    // LED - Backlight LED, pin 8 on LCD.
...
// sends the data byte
void LCDWrite(byte data_or_command, byte data, int displayNumber) {
  if (displayNumber == 0) {
    digitalWrite(dcPin0, data_or_command);
    digitalWrite(scePin0, LOW);
    SPI.transfer(data);  //shiftOut(sdinPin, sclkPin, MSBFIRST, data);
    digitalWrite(scePin0, HIGH);
  }
  if (displayNumber == 1) {
    digitalWrite(dcPin1, data_or_command);
    digitalWrite(scePin1, LOW);
    SPI1.transfer(data);  //shiftOut(sdinPin, sclkPin, MSBFIRST, data);
    digitalWrite(scePin1, HIGH);
  }
}
...
void gotoXY(int x, int y, int displayNumber) {
  LCDWrite(0, 0x80 | x, displayNumber);  // Column.
  LCDWrite(0, 0x40 | y, displayNumber);  // Row.  ?
}

// This will actually draw on the display, whatever is currently
// in the displayMap array.
void updateDisplay(int displayNumber) {
  gotoXY(0, 0, displayNumber);
  for (int i = 0; i < (LCD_WIDTH * LCD_HEIGHT / 8); i++) {
    LCDWrite(LCD_DATA, displayMap[i], displayNumber);
  }
}

// Set contrast can set the LCD Vop to a value between 0 and 127.
// 40-60 is usually a pretty good range.
void setContrast(byte contrast, int displayNumber) {
  LCDWrite(LCD_COMMAND, 0x21, displayNumber);             //Tell LCD that extended commands follow
  LCDWrite(LCD_COMMAND, 0x80 | contrast, displayNumber);  //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark
  LCDWrite(LCD_COMMAND, 0x20, displayNumber);             //Set display mode
}
...
you please. */
void invertDisplay(int displayNumber) {
...
void lcdBegin(int displayNumber) {
  if (displayNumber == 0) {
    //Configure control pins
    pinMode(scePin0, OUTPUT);
    pinMode(rstPin0, OUTPUT);
    pinMode(dcPin0, OUTPUT);
    pinMode(sdinPin0, OUTPUT);
    pinMode(sclkPin0, OUTPUT);
    pinMode(blPin0, OUTPUT);
    analogWrite(blPin0, 255);
    ...
    digitalWrite(rstPin0, LOW);
    digitalWrite(rstPin0, HIGH);

    LCDWrite(LCD_COMMAND, 0x21, 0);  //Tell LCD extended commands follow
    LCDWrite(LCD_COMMAND, 0xB0, 0);  //Set LCD Vop (Contrast)
    LCDWrite(LCD_COMMAND, 0x04, 0);  //Set Temp coefficent
    LCDWrite(LCD_COMMAND, 0x14, 0);  //LCD bias mode 1:48 (try 0x13)
    //We must send 0x20 before modifying the display control mode
    LCDWrite(LCD_COMMAND, 0x20, 0);
    LCDWrite(LCD_COMMAND, 0x0C, 0);  //Set display control, normal mode.
  }
  if (displayNumber == 1) {
    //Configure control pins
    pinMode(scePin1, OUTPUT);
    pinMode(rstPin1, OUTPUT);
    pinMode(dcPin1, OUTPUT);
    pinMode(sdinPin1, OUTPUT);
    pinMode(sclkPin1, OUTPUT);
    pinMode(blPin1, OUTPUT);
    analogWrite(blPin1, 255);
    ...
    LCDWrite(LCD_COMMAND, 0x21, 1);  //Tell LCD extended commands follow
    LCDWrite(LCD_COMMAND, 0xB0, 1);  //Set LCD Vop (Contrast)
    LCDWrite(LCD_COMMAND, 0x04, 1);  //Set Temp coefficent
    LCDWrite(LCD_COMMAND, 0x14, 1);  //LCD bias mode 1:48 (try 0x13)
    //We must send 0x20 before modifying the display control mode
    LCDWrite(LCD_COMMAND, 0x20, 1);
    LCDWrite(LCD_COMMAND, 0x0C, 1);  //Set display control, normal mode.
  }
}